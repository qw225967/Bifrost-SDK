...
namespace RTCUtils {
		...
		CopyOnWriteBuffer::CopyOnWriteBuffer(CopyOnWriteBuffer&& buf)
		    : buffer_(std::move(buf.buffer_)), offset_(buf.offset_),
		      size_(buf.size_) {
				buf.offset_ = 0;
				buf.size_   = 0;
		}
		...
		CopyOnWriteBuffer::CopyOnWriteBuffer(size_t size)
		    : buffer_(size > 0 ? new RefCountedBuffer(size) : nullptr), offset_(0),
		      size_(size) {
		}
		...
		bool CopyOnWriteBuffer::operator==(const CopyOnWriteBuffer& buf) const {
				// Must either be the same view of the same buffer or have the same
				// contents.
		...
		void CopyOnWriteBuffer::UnshareAndEnsureCapacity(size_t new_capacity) {
				...
				buffer_ = new RefCountedBuffer(buffer_->data() + offset_, size_,
				                               new_capacity);
				offset_ = 0;
		}
