...
#ifndef NEBULA_BASE_COPY_ON_WRITE_BUFFER_H_
#define NEBULA_BASE_COPY_ON_WRITE_BUFFER_H_
...
namespace RTCUtils {
		...
		public:
				...
				// The source array may be (const) uint8_t*, int8_t*, or char*.
				template<typename T,
				...
				// Get a pointer to the data. Just .data() will give you a (const)
				// uint8_t*, but you may also use .data<int8_t>() and .data<char>().
				...
				// Get writable pointer to the data. This will create a copy of the
		...
		private:
				...
				// Create a copy of the underlying data if it is referenced from other
				// Buffer objects or there is not enough capacity.
				...
				size_t size_; // Size of a current slice in the original data in
				              // buffer_. Should be 0 if the buffer_ is empty.
		};
} // namespace RTCUtils

#endif // NEBULA_BASE_COPY_ON_WRITE_BUFFER_H_
