 ...
 */
#ifndef NEBULA_API_REF_COUNTED_BASE_H_
#define NEBULA_API_REF_COUNTED_BASE_H_
...
namespace RTCUtils {
		...
		private:
				mutable RTCUtils::RefCounter ref_count_{ 0 };
		};

		// Template based version of `RefCountedBase` for simple implementations
		// that do not need (or want) destruction via virtual destructor or the
		// overhead of a vtable.
		//
		// To use:
		...
		public:
				...
				RefCountReleaseStatus Release() const {
						...
						// options:
						// 1) The class doesn't actually need virtual methods, the type is
						// complete
						//    so the virtual attribute(s) can be removed.
						// 2) The virtual methods are a part of the design of the class. In this
						//    case you can consider using `RefCountedBase` instead or
						//    alternatively use `rtc::RefCountedObject`.
						static_assert(
...
#endif // NEBULA_API_REF_COUNTED_BASE_H_
