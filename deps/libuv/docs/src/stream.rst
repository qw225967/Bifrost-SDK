...
.. c:type:: void (*uv_read_cb)(uv_stream_t* stream, ssize_t n_read, const uv_buf_t* buf)

    Callback called when data was read on a stream.

    `n_read` is > 0 if there is data available or < 0 on error. When we've
    reached EOF, `n_read` will be set to ``UV_EOF``. When `n_read` < 0,
    the `buf` parameter might not point to a valid buffer; in that case
    `buf.len` and `buf.base` are both set to 0.

    .. note::
        `n_read` might be 0, which does *not* indicate an error or EOF. This
