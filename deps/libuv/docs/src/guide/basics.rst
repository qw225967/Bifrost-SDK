...
I/O read callbacks (such as for files and sockets) are passed a parameter ``n_read``. If ``n_read`` is less than 0, there was an error (UV_EOF is the end of file error, which you may want to handle differently).
