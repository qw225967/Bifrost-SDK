...
The read callback ``n_read`` parameter is less than 0 on any error. This error
...
Otherwise ``n_read`` is a non-negative number and we can attempt to write that
...
The read callback may be called with ``n_read = 0``, indicating that at this
