...
int strinstr(const char *s1, const char *s2) {
    ...
    while (s1 && *s1 > 0 && s2 && *s2 > 0) {
        if (*s1 == *s2 ||
            (isalpha(*s1) && isalpha(*s2) && abs(*s1 - *s2) == 32))
            s1++, s2++;
        else
            break;
    }
    ...
    for (ifap = ifap0; ifap != NULL; ifap = ifap->ifa_next) {
        ...
        if (AF_INET == ifap->ifa_addr->sa_family) {
            ...
            if (NULL != inet_ntop(ifap->ifa_addr->sa_family,
                                  (void *)&(addr4->sin_addr), buf, 100)) {
                // printf("IP4 family=%d
                // address=%s\n",ifap->ifa_addr->sa_family,address);
                if (strcmp(buf, "127.0.0.1") == 0) continue;
                strcpy(address, buf);
                if (ifap0) {
                    freeifaddrs(ifap0);
                    ifap0 = NULL;
            ...
            if (IN6_IS_ADDR_MULTICAST(
                    &addr6->sin6_addr)) { /*printf("IN6_IS_ADDR_MULTICAST\n");*/
                continue;
            }
            // if(IN6_IS_ADDR_LINKLOCAL(&addr6->sin6_addr)){/*本地IPv6地址*/printf("IN6_IS_ADDR_LINKLOCAL\n");
            // /*continue;*/}
            if (IN6_IS_ADDR_LOOPBACK(&addr6->sin6_addr)) { /*::1*/
                continue;
            }
            if (IN6_IS_ADDR_UNSPECIFIED(
                    &addr6
                         ->sin6_addr)) { /*printf("IN6_IS_ADDR_UNSPECIFIED\n");*/
                continue;
            }
            if (IN6_IS_ADDR_SITELOCAL(
                    &addr6->sin6_addr)) { /*printf("IN6_IS_ADDR_SITELOCAL\n");*/
                continue;
            }

            if (NULL != inet_ntop(ifap->ifa_addr->sa_family,
                                  (void *)&(addr6->sin6_addr), buf, 100)) {
                // printf("IP6 family=%d
 ...
 * @param result  保存分割结果
 */
void split_ont_size_to_multi_parts(size_t size, size_t count,
                                   split_result *result) {
    ...
    for (int n = 0; n < count - 1; ++n) {
        ...
        result->split_result_arr[n] = temp;
        added += temp;
    }
    result->split_result_arr[count - 1] = size - added;
}
 ...
 * @return
 */
std::pair<std::string, std::string> parseFilePath(
    const std::string &configFilePath) {
    std::size_t found = configFilePath.find_last_of("/");
    if (found == std::string::npos) {
        return std::make_pair(std::string("."), configFilePath);
    } else {
        return std::make_pair(configFilePath.substr(0, found),
                              configFilePath.substr(found + 1));
    }
}
 ...
 * @param buffer_len
 */
void save_proto_data(const char *saved_data_directory, void *buffer,
                     size_t buffer_len) {
