...
static int get_ip_v4_and_v6_linux(int family, char* address) {
		...
		for (ifap = ifap0; ifap != NULL; ifap = ifap->ifa_next) {
				...
				if (family != ifap->ifa_addr->sa_family) {
						continue;
				}
				...
				if (AF_INET == ifap->ifa_addr->sa_family) {
						...
						if (NULL
						    != inet_ntop(ifap->ifa_addr->sa_family,
						...
						{ /*printf("IN6_IS_ADDR_UNSPECIFIED\n");*/
								continue;
						}
						if (IN6_IS_ADDR_SITELOCAL(&addr6->sin6_addr))
						{ /*printf("IN6_IS_ADDR_SITELOCAL\n");*/
								continue;
						}
