...
#define ISSET_FLAG(flag, value) ((flag) & (value))
...
typedef struct websocket_handle {
    membuf_t proto_buf;           /// 原始帧数据
    membuf_t merge_buf;           /// 需要缓存的多个分片
    endpoint_type endpoint_type;  /// 在哪个端创建的句柄

    uchar is_eof : 1;  /// 是否是结束帧 data[0]>>7
    uchar df_ext : 3;  /// 是否有扩展定义 (data[0]>>4) & 0x7
    uchar type : 4;    /// 消息类型

    uchar has_mask : 1;     /// 是否有掩码
    uchar payload_len : 7;  /// payload len field

    uchar mask[4];  /// 掩码内容

    size_t real_payload_len;  /// 真实数据长度
    size_t read_count;        /// 本次解析读取的字节数量
    parse_state parse_state;  /// 解析状态
} websocket_handle;
 ...
 */
// void parse_websocket_frame(websocket_handle* handle, char* data, ulong
// datalen);
